...
namespace MyTools {
    ...
    void __fastcall RealFileLoggerSingletone::OpenLogFile(const string& FN)
    {
        logOut.open(FN, ios_base::out);
    }

    void RealFileLoggerSingletone::CloseLogFile()
    {
        if (logOut.is_open())
        {
            logOut.close();
    ...
    void __fastcall RealFileLoggerSingletone::WriteToLog(const string& str)
    {
        if (logOut.is_open())
        {
    ...
    void __fastcall RealFileLoggerSingletone::WriteToLog(const string& str, int n)
    {
        if (logOut.is_open())
        {
    ...
    void __fastcall RealFileLoggerSingletone::WriteToLog(const string& str, double d)
    {
        if (logOut.is_open())
        {
    ...
    void __fastcall LoggerSingletone::WriteToLog()
    {
        //LoggerSingletone::iter = 0;
        if (logOut.is_open())
        {
            logOut << LoggerSingletone::iter << ".   ";
        }
        LoggerSingletone::iter++;
    }
